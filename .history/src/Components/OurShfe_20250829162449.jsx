    ...
    <div className="bg-[#f7f4ed] py-20">
      ...
      <div className="container mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 px-5">
        {chefs.map((chef) => (
          <div
            key={chef.id}
            className="bg-white rounded-t-[200px] shadow-lg overflow-hidden text-center relative"
          >
            {/* Image Wrapper */}
            <div className="">
              <img
                src={chef.img}
                alt={chef.name}
                className={`w-[312px] h-[288px] absolute  rounded-b-[100px] border-b-4 ${chef.borderColor}`}
              />
              {/* Share Button */}
              <button
