...
console.log(cardItems);
  ...
  try {
    ...
    const orderData = {
      userId: user._id,
      shipping: formData,    // form data
      items: cardItems,      // cart items
      createdAt: new Date()
    };

    // Step 2: send to backend to save in orders collection
    const saveOrder = await axios.post("http://localhost:5000/orders", orderData);

    if (saveOrder.data.success) {
      // Step 3: delete cart items for this user
      await axios.delete(`http://localhost:5000/cardItems`, {
        params: { userId: user._id },
      });
