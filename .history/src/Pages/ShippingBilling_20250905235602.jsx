...
const ShippingBilling = () => {
  ...
  const allInfo ={
    ...
    try {
      const response = await axios.delete(`http://localhost:5000/orderInfo/${id}`);
      if (response.data.success) {
  ...
  return (
    <div className="max-w-5xl mx-auto mt-10 grid grid-cols-3 gap-6">
      ...
      <div className="bg-white rounded-2xl shadow p-4 h-fit space-y-4">
        ...
        <button onClick={handlePayment} className="w-full bg-orange-500 text-white py-2 rounded font-medium hover:bg-orange-600">
          Proceed to Pay
        </button>
      </div>
    </div>
