...
const ShippingBilling = () => {
    ...
    const fetchItem = async () => {
      try {
        ...
        if (response.data.success) {
          setItems(response.data.data); // ধরলাম orderInfo API থেকে array আসে
        } else {
          setItems([]);
        }
      } catch (error) {
  ...
  // Total হিসাব করা
  const itemsTotal = items.reduce((acc, item) => acc + (item.itemPrice * (item.quantity || 1)), 0);
