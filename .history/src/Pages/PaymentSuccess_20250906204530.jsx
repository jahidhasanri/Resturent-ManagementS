...
const PaymentSuccess = () => {
  ...
  const handleDownload = () => {
    ...
    // **Free html2canvas safe approach:** 
    // Convert all oklch colors (if any) to hex
  ...
  return (
    <div className="mt-20 mb-10">
      <h1 className="text-2xl text-center font-bold text-green-600">Payment Successful ✅</h1>
      ...
      >
        ...
        <div className="mx-[100px] my-[50px] border-[7px] border-[#925a13] relative z-10">
          <div className="relative h-1/3 flex items-center justify-center p-4"
            style={{
            ...
            <div className="relative w-full z-10 flex flex-col items-center justify-center">
              <img src="/public/images/ChatGPT_Image_Aug_28__2025__12_17_00_PM-removebg-preview.png" alt="Logo" className="h-14" />
              ...
              <div className="flex w-full items-end gap-[200px] justify-between mt-4 px-8">
                <div className="text-white text-start flex-1">
                  ...
                  <h5 className="text-[15px] font-semibold mb-1">
                    <span className="text-[16px] font-bold">Name: </span> {shipping?.fullName}
                  </h5>
                  <h5 className="text-[15px] font-semibold mb-1">
                    <span className="text-[16px] font-bold">Area: </span> {shipping?.area}
                  </h5>
                  <h5 className="text-[15px] font-semibold">
                    <span className="text-[16px] font-bold">Building: </span> {shipping?.building}
                  </h5>
                </div>
                <div className="text-white flex-1 text-start">
                  <h5 className="text-[15px] font-semibold mb-1">
                    <span className="text-[16px] font-bold">Phone:</span> {shipping?.phoneNumber}
                  </h5>
                  <h5 className="text-[15px] font-semibold mb-1">
                    <span className="text-[16px] font-bold">Address:</span> {shipping?.address}
                  </h5>
                  <h5 className="text-[15px] font-semibold mb-1">
                    <span className="text-[16px] font-bold">City:</span> {shipping?.city}
                  </h5>
                </div>
              </div>
            </div>
          ...
          <div className="p-6 bg-black bg-opacity-80 text-white">
                ...
                {items?.map((item, idx) => (
                  <tr key={idx} className="border-b border-gray-700">
                    ...
                    <td className="px-4 py-2">
                      <img className="w-[80px] h-[80px]" src={item?.itemImg} alt="" />
                    </td>
                    <td className="px-4 py-2 text-center">{item.itemPrice} ৳</td>
                    <td className="px-4 py-2 text-center">{item?.quantity}</td>
                    <td className="px-4 py-2 text-center">{item.itemPrice * item?.quantity} ৳</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="flex gap-2 mt-5 items-center">
              ...
              <span className="text-green-700 text-[20px] font-semibold">{data?.paidstatus}</span>
            </div>
      ...
      <div className="mt-8 flex justify-center gap-6">
        <button onClick={() => navigate("/")} className="px-6 py-3 rounded bg-blue-500 text-white hover:bg-blue-600">
          Back To Home
        </button>
        <button onClick={handleDownload} className="px-6 py-3 rounded bg-green-500 text-white hover:bg-green-600">
