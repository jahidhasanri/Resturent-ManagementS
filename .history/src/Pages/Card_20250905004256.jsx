    ...
    <div className=" p-4">
      ...
      <div className="container mx-auto">
        {cardItems.length === 0 ? (
         ...
         <table className="w-full mt-10 border-collapse border border-gray-300" style={{ borderSpacing: "0 2rem" }}>
            ...
            <tbody>
              {cardItems.map((item) => (
                <tr key={item.itemId} className="text-center mb-20">
                  <td className=" px-4 py-2 mb-20">
                    <img
                      src={item.itemImg}
                      alt={item.itemName}
                  ...
                  <td className=" px-4 py-2 mb-20">{item.itemName}</td>
                  <td className=" px-4 py-2
                  ">${item.itemPrice}</td>
                  <td className=" px-4 py-2 flex items-center justify-center gap-2">
                    <button
                      onClick={() => handleQuantityChange(item.itemId, -1)}
                      className="px-2 py-1 border rounded mt-5"
                    >
                      −
                    </button>
                    <span className="mt-5">{item.quantity || 1}</span>
                    <button
                      onClick={() => handleQuantityChange(item.itemId, 1)}
                      className="px-2 py-1 border rounded mt-5"
                    >
