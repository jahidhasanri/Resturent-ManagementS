...
const PaymentSuccess = () => {
  ...
  // calculate total
  const totalAmount = data?.orders[0]?.items?.reduce(
  ...
  <tr key={idx} className="odd:bg-gray-50 even:bg-white">
    ...
    <td className="border px-4 py-2">
      <img src={item.itemImg} alt={item.itemName} className="h-12 w-12 object-cover rounded" />
    </td>
    <td className="border px-4 py-2">{item.itemPrice} ৳</td>
    <td className="border px-4 py-2">{item.quantity}</td>
    <td className="border px-4 py-2">{item.itemPrice * item.quantity} ৳</td>
  </tr>
))}
              </tbody>
            </table>
